...
| `wmcaSetOrderPwd` | 거래비밀번호1/2 해시 생성 | 현재 구현에서 `s8180`/`s8118` 입력에 연결됨 |
...
- 거래비밀번호 원문은 숫자 문자열로 입력받되, TR에는 `wmcaSetOrderPwd` 결과 44바이트를 넣는다.
...
- `QV_ACCOUNT_PASSWORD`: 계좌별 4자리 계좌비밀번호
- `QV_TRADE_PASSWORD`: 세션 공통 거래비밀번호
- `QV_TRADE_PASSWORD1`, `QV_TRADE_PASSWORD2`: 필요 시 개별 override

현재 구현은 `s8180`/`s8118`에서 아래처럼 동작한다.

- `pswd_noz44` = 계좌비밀번호 해시
- `trad_pswd1z44`, `trad_pswd2z44` = 거래비밀번호 해시
...
- 로그인은 성공한다.
- `wmcaSetAccountIndexPwd`, `wmcaSetAccountNoPwd`, `wmcaSetOrderPwd` 호출 로그를 남기고 있다.
- 현재 구현은 `trad_pswd1/2`에도 거래비밀번호 해시를 채운다.
- 그런데 여전히 `s8180`에서 `21263 계좌비밀번호를 잘못 입력하셨습니다.`가 반환될 수 있다.

즉, 현재 남은 원인 후보는 아래 셋이다.

1. 선택한 계좌 자체가 `s8180` 조회 대상이 아닌데 UI에서 구분 없이 노출된다.
2. 해당 계좌는 `wmcaSetAccountIndexPwd`가 아니라 `wmcaSetAccountNoPwd` 기준 해시가 맞다.
3. 계좌/거래비밀번호를 모두 채운 뒤에도 해당 계좌 또는 TR 조합이 서버 기준으로 거절된다.
...
| 순번 | 항목 | 타입 | 크기 | 비고 |
| --- | --- | --- | --- | --- |
| 1 | 계좌비밀번호 | char | 44 | 계좌비밀번호 해시 |
| 2 | 잔고구분 | char | 1 | 기본 `1` 체결기준 |
| 3 | 자산기준 | char | 1 | 기본 `1` 순자산 |
| 4 | 시세구분코드 | char | 3 | 기본 `UNT` |
...
- `LOGIN`
- `QUERY`
  - 신포맷: `trade_password` 포함
  - 구포맷: `selected_count`만 포함
- `BALANCE`

`QUERY`는 세션 공통 거래비밀번호를 별도 줄로 전달하고, 각 selection line은 계속 `account_index + account_no + account_password`만 보낸다.
...
## 8. 현재 확정된 것 vs 미확정인 것

### 확정된 것

- 로그인 자격증명 자체는 맞다.
- `s8180` 실패는 로그인 이후 특정 계좌 조회에서만 난다.
- `wmcaSetAccountIndexPwd` / `wmcaSetAccountNoPwd` / `wmcaSetOrderPwd` 호출 자체는 되고 있다.
- 예전 가설이던 “해시를 `std::string`으로 복사하면서 깨진다”는 것은 현재 로그로는 주원인이 아니다.
- 현재 구현은 `s8180`/`s8118`에 거래비밀번호 해시를 명세대로 주입한다.
...
| `QV_TRADE_PASSWORD` | 숫자 문자열 | 세션 공통 거래비밀번호 |
| `QV_TRADE_PASSWORD1` / `QV_TRADE_PASSWORD2` | 숫자 문자열 | `QV_TRADE_PASSWORD`보다 우선하는 개별 override |
...
```json
{
  ...
  "s8180_diagnostic": {
    ...
    "trade_password_present": true,
    "trade_password_length": 4,
    "trade_hash_generation_ok": true,
    "query_submitted": true,
    "query_succeeded": true,
    "hash_source": "wmcaSetAccountNoPwd",
    "trade_hash_source": "wmcaSetOrderPwd",
...
- `account_password_rejected` + `credential_rejected_after_full_password_payload`
  - `index`, `account_no` 모두 실패했고 거래비밀번호까지 해시 주입된 상태에서 거절
- `account_password_rejected` + `binding_mismatch_or_ineligible_account`
  - 단일 바인딩 실험에서 계좌비밀번호 오류
- `missing_trade_password`
  - 거래비밀번호 없이 `s8180`을 실행하려고 했음
- `trade_password_hash_generation_failed`
  - `wmcaSetOrderPwd` 경로 실패
